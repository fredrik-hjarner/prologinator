...
if (!MODULE) {
    ...
    console.error("Usage: bun scripts/test_trealla.ts <module_path>");
    process.exit(1);
}
...
const tmpFile = join(tmpdir(), `${base}.gpp.tpl.pl`);
...
// Check for test framework specific failure message
if (output.includes("Failed")) {
    console.error(`\n❌ ERROR: test failed`);
    process.exit(1);
}

// Scryer often uses " causes: ", Trealla often uses "Error:" or "exception:"
if (
    output.includes(" causes: ") ||
